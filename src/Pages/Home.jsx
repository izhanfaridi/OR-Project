  ...
  const calculate = () => {
    ...
    if (serverQty > 1) {
      if (interArrivalType === "M" && serviceType === "M") {
        ...
        const ggc_answer = ggc_calculation(
          p,
          lambda,
          meu,
          serverQty,
      ...
      if (interArrivalType === "M" && serviceType === "M") {
        ...
        const gg1_answer = gg1_calculation(
          p,
          lambda,
          meu,
          variance_S,
  ...
  return (
    ...
    <div className="h-screen bg-gradient-to-bl from-[#21252B] to-[#2A323D] to-[#2D3C55]">
      <BsFillFileBarGraphFill
        ...
        className="absolute right-28 text-5xl text-white top-8 hover:text-gray-400"
      />
      <SiMicrosoftexcel
        onClick={() => {
          navigate("/graphs");
        }}
        className="absolute right-10 text-5xl text-white top-8 hover:text-gray-400"
      />
      <div className="bg-gradient-to-bl from-[#21252B] to-[#2A323D] to-[#2D3C55] flex flex-row flex-wrap justify-evenly content-start items-baseline h-full py-5">
        <h1 className="text-center text-white text-2xl py-6 font-bold">
          Operation Research Simulator
        </h1>
        <div className="border-t w-full flex flex-row justify-center items-center py-6">
          ...
          <input
            className="mx-4 rounded-md border border-black p-2 text-md focus:outline-none"
        ...
        <div className="border rounded-md text-white w-1/4 p-5">
          ...
          <select
            className="text-[#1A1D21] rounded-md w-2/5 h-10 focus:outline-none"
          ...
          <h3 className="text-lg font-bold py-5">
            Mean of Inter Arrival Time (λ)
          </h3>
          <input
            type="number"
            placeholder="Enter λ Value In Minutes"
            className="text-[#1A1D21] rounded-md w-3/5 h-10 focus:outline-none pl-2"
            onChange={(e) => setLambda(1 / parseFloat(e.target.value))}
          />

          <h3 className="text-lg font-bold py-5">
            Variance of Inter Arrival ( σ<sup>2</sup>
            <sub>a</sub> )
          </h3>
          <input
            type="number"
            disabled={interArrivalType === "M" ? true : false}
            placeholder="Enter σ Value In min^2"
            className="text-[#1A1D21] rounded-md w-3/5 h-10 focus:outline-none pl-2"
            onChange={(e) => setVariance_A(parseFloat(e.target.value))}
          />
        </div>
        <div className="border rounded-md text-white w-1/4 p-5">
          ...
          >
            ...
            <option hidden={interArrivalType === "G" ? true : false} value="M">
              Exponential
            </option>
            <option value="G">Uniform</option>
            <option value="G">Gamma</option>
          ...
          <input
            type="number"
            placeholder="Enter μ Value In Minutes"
            className="text-[#1A1D21] rounded-md w-3/5 h-10 focus:outline-none pl-2"
            onChange={(e) => setMeu(1 / parseFloat(e.target.value))}
          />

          <h3 className="text-lg font-bold py-5">
            Variance of Service Time ( σ<sup>2</sup>
            <sub>s</sub> )
          </h3>
          <input
            type="number"
            disabled={serviceType === "M" ? true : false}
            placeholder="Enter σ Value In min^2"
            className="text-[#1A1D21] rounded-md w-3/5 h-10 focus:outline-none pl-2"
            onChange={(e) => setVariance_S(parseFloat(e.target.value))}
          />
        </div>
        <div className="w-full flex justify-center pt-8">
          {Loading ? (
            ...
            <button
              onClick={calculate}
              className="rounded-md bg-[#2E71FF] text-white font-bold w-1/4 h-12 text-center text-lg"
            >
              CALCULATE
            </button>
          )}
